...
import com.mp3song.playmusic.helpers.M3UWriter;
import com.mp3song.playmusic.models.Playlist;
import com.mp3song.playmusic.models.PlaylistSong;
...
public class PlaylistsUtil {
    ...
    public static int createPlaylist(@NonNull final Context context, @Nullable final String name) {
            ...
            try {
                ...
                if (cursor == null || cursor.getCount() < 1) {
                    ...
                    if (uri != null) {
                        ...
                        Toasty.success(context, context.getResources().getString(
                                R.string.created_playlist_x, name), Toasty.LENGTH_SHORT).show();
        ...
        if (id == -1) {
            Toasty.error(context, context.getResources().getString(
                    R.string.could_not_create_playlist), Toasty.LENGTH_SHORT).show();
        }
        return id;
    }
    ...
    public static void addToPlaylist(@NonNull final Context context, @NonNull final List<Song> songs, final int playlistId, final boolean showToastOnFinish) {
        ...
        try {
            ...
            if (showToastOnFinish) {
                Toasty.success(context, context.getResources().getString(
                        R.string.inserted_x_songs_into_playlist_x, numInserted, getNameForPlaylist(context, playlistId)), Toasty.LENGTH_SHORT).show();
                LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(context);
                Intent localIn;
                localIn = new Intent("REFRESH_PLAYLIST");
                lbm.sendBroadcast(localIn);
    ...
    public static void removeFromPlaylist(@NonNull final Context context, @NonNull final Song song, int playlistId) {
        ...
        Toasty.info(context, context.getString(R.string.remove_x_songs, song.title), Toasty.LENGTH_SHORT).show();
        LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(context);
        Intent localIn;
        localIn = new Intent("REFRESH_PLAYLIST_WHOLE");
        lbm.sendBroadcast(localIn);
    }
