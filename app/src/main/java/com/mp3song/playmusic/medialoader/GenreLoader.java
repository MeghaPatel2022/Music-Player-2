...
public class GenreLoader {
    ...
    public static ArrayList<Genre> getGenreForAlbum(Context context, @NonNull long albumID) {
        // a list of song that belong to an artist
        Cursor cursor = GenreLoader.getGenreCursorForAlbum(context, albumID);
        return GenreLoader.getGenresFromCursor(context, cursor);
    }

    public static Cursor getGenreCursorForAlbum(final Context context, long albumID) {
        String query = " id in (select genre_id from audio_genres_map where audio_id in (select id from audio_meta where " + SongLoader.BASE_SELECTION + " AND album_id = " + albumID + "))";
        final String[] projection = new String[]{
                Genres._ID,
                Genres.NAME
        };
